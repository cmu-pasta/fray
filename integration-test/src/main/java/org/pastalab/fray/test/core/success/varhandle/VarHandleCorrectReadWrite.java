...
public class VarHandleCorrectReadWrite {
  ...
  public static void main(String[] args)
    ...
    // short
    var VH_SHORT =
        MethodHandles.lookup().findVarHandle(SimpleObject.class, "shortValue", short.class);
    ...
    // float
    var VH_FLOAT =
        MethodHandles.lookup().findVarHandle(SimpleObject.class, "floatValue", float.class);
    VH_FLOAT.setVolatile(obj, 3.14f);
    assert ((Float) VH_FLOAT.getVolatile(obj)).floatValue() == 3.14f;

    // double
    var VH_DOUBLE =
        MethodHandles.lookup().findVarHandle(SimpleObject.class, "doubleValue", double.class);
    ...
    // boolean
    var VH_BOOLEAN =
        MethodHandles.lookup().findVarHandle(SimpleObject.class, "booleanValue", boolean.class);
    VH_BOOLEAN.setVolatile(obj, true);
    assert ((Boolean) VH_BOOLEAN.getVolatile(obj)).booleanValue();

    // int[]
    var VH_INT_ARRAY =
        MethodHandles.lookup().findVarHandle(SimpleObject.class, "intArray", int[].class);
