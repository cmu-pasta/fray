...
class ThreadTimelinePanel : JPanel() {
  private val threadExecutionHistory = mutableMapOf<Long, MutableList<Pair<Int, String>>>()
  ...
  init {
    ...
    threadExecutionHistory[0] = mutableListOf(Pair(currentTime++, "Program Start"))
  ...
  fun newThreadScheduled(thread: ThreadExecutionContext) {
    ...
    threadExecutionHistory
        .getOrPut(thread.threadInfo.index) { mutableListOf() }
  ...
  inner class ThreadTimelineCanvas : JPanel() {
    ...
    init {
      ...
      addMouseMotionListener(object : MouseMotionAdapter() {
        override fun mouseMoved(e: MouseEvent) {
          ...
          for ((threadIndex, events) in threadExecutionHistory) {
    ...
    override fun getPreferredSize(): Dimension {
      ...
      val maxTimeStep = threadExecutionHistory.values.flatMap { it }.maxOfOrNull { it.first } ?: 0
    ...
    override fun paintComponent(g: Graphics) {
      ...
      // Draw thread names and timelines
      ...
      threadExecutionHistory.entries.forEachIndexed { index, entry ->
        ...
        g2d.drawString("Thread $threadIndex", 10, y + 5)

        // Draw timeline for this thread
        drawThreadTimeline(g2d, threadIndex, events, y)
      }
    }

    private fun drawThreadTimeline(g2d: Graphics2D, threadIndex: Long, events: List<Pair<Int, String>>, y: Int) {
      ...
      events.forEachIndexed { index, event ->
        ...
        // Draw event marker
        val isHovered = hoveredEvent?.let { it.first == threadIndex && it.second.first == timeStep } ?: false
        ...
        if (events.size < 20) {
          g2d.color = JBColor.foreground()
          g2d.font = Font(Font.SANS_SERIF, Font.PLAIN, 9)
