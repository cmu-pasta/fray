...
```kotlin
plugins {
    id("org.pastalab.fray.gradle") version "0.7.1"
}
```
...
```dockerfile
FROM ghcr.io/cmu-pasta/fray:0.7.1 as fray
...
- Add the following two agents:
  - `-javaagent:/opt/fray/libs/fray-core-FRAY_VERSION-SNAPSHOT-all.jar=FRAY_ARGS`
    ...
    - For example, to use the `pos` scheduler and enable memory interleaving:
      ```
      -javaagent:/opt/fray/libs/fray-core-0.7.1-SNAPSHOT-all.jar=-m:--scheduler:pos
      ```
